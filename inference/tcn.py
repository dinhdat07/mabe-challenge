"""Inference loop for TCN ensemble (causal) matching notebook logic."""
...
from mabe.training.tcn_trainer import MouseTCN


def load_tcn_resources(base_dir: Path, n_folds: int, device: str = "cuda"):
    ...
    for fold in range(n_folds):
        ...
        first_w = state[[k for k in state if k.endswith("weight")][0]]
        n_feat = first_w.shape[1]
        last_w = state[[k for k in state if k.endswith("weight")][-1]]
        n_class = last_w.shape[0]
        model = MouseTCN(n_feat=n_feat, n_class=n_class).to(device)
...
def submit(
    ...
    *,
    ...
    pack = load_tcn_resources(section_dir, n_folds=n_folds, device=device)
